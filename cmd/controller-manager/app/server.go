...
import (
	...
	"imaginekube.com/imaginekube/pkg/apis"
	controllerconfig "imaginekube.com/imaginekube/pkg/apiserver/config"
	"imaginekube.com/imaginekube/pkg/controller/cluster"
	"imaginekube.com/imaginekube/pkg/controller/network/webhooks"
	...
	"imaginekube.com/imaginekube/pkg/utils/term"
	"imaginekube.com/imaginekube/pkg/version"
)

func NewControllerManagerCommand() *cobra.Command {
	s := options.NewImagineKubeControllerManagerOptions()
	conf, err := controllerconfig.TryLoadFromDisk()
	if err == nil {
		// make sure LeaderElection is not nil
		s = &options.ImagineKubeControllerManagerOptions{
	...
	cmd := &cobra.Command{
		...
		Long: `ImagineKube controller manager is a daemon that embeds the control loops shipped with ImagineKube.`,
	...
	versionCmd := &cobra.Command{
		Use:   "version",
		Short: "Print the version of ImagineKube controller-manager",
		Run: func(cmd *cobra.Command, args []string) {
...
func Run(s *options.ImagineKubeControllerManagerOptions, configCh <-chan controllerconfig.Config, ctx context.Context) error {
...
func run(s *options.ImagineKubeControllerManagerOptions, ctx context.Context) error {
	...
	if s.LeaderElect {
		mgrOptions = manager.Options{
			...
			LeaderElectionNamespace: "imaginekube-system",
	...
	if s.IsControllerEnabled("cluster") && s.MultiClusterOptions.Enable {
		hookServer.Register("/validate-cluster-imaginekube-io-v1alpha1", &webhook.Admission{Handler: &cluster.ValidatingHandler{Client: mgr.GetClient()}})
	}
	hookServer.Register("/validate-email-iam-imaginekube-io-v1alpha2", &webhook.Admission{Handler: &user.EmailValidator{Client: mgr.GetClient()}})
	hookServer.Register("/validate-network-imaginekube-io-v1alpha1", &webhook.Admission{Handler: &webhooks.ValidatingHandler{C: mgr.GetClient()}})
	hookServer.Register("/mutate-network-imaginekube-io-v1alpha1", &webhook.Admission{Handler: &webhooks.MutatingHandler{C: mgr.GetClient()}})
	...
	hookServer.Register("/validate-quota-imaginekube-io-v1alpha2", &webhook.Admission{Handler: resourceQuotaAdmission})
