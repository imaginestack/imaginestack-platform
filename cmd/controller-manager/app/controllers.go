...
import (
	...
	"github.com/imaginekube/pvc-autoresizer/runners"
	...
	"imaginekube.com/imaginekube/cmd/controller-manager/app/options"
	"imaginekube.com/imaginekube/pkg/controller/alerting"
	"imaginekube.com/imaginekube/pkg/controller/application"
	"imaginekube.com/imaginekube/pkg/controller/certificatesigningrequest"
	"imaginekube.com/imaginekube/pkg/controller/cluster"
	"imaginekube.com/imaginekube/pkg/controller/clusterrolebinding"
	"imaginekube.com/imaginekube/pkg/controller/destinationrule"
	"imaginekube.com/imaginekube/pkg/controller/globalrole"
	"imaginekube.com/imaginekube/pkg/controller/globalrolebinding"
	"imaginekube.com/imaginekube/pkg/controller/group"
	"imaginekube.com/imaginekube/pkg/controller/groupbinding"
	"imaginekube.com/imaginekube/pkg/controller/helm"
	"imaginekube.com/imaginekube/pkg/controller/job"
	"imaginekube.com/imaginekube/pkg/controller/loginrecord"
	"imaginekube.com/imaginekube/pkg/controller/namespace"
	"imaginekube.com/imaginekube/pkg/controller/network/ippool"
	"imaginekube.com/imaginekube/pkg/controller/network/nsnetworkpolicy"
	"imaginekube.com/imaginekube/pkg/controller/network/nsnetworkpolicy/provider"
	"imaginekube.com/imaginekube/pkg/controller/notification"
	"imaginekube.com/imaginekube/pkg/controller/openpitrix/helmapplication"
	"imaginekube.com/imaginekube/pkg/controller/openpitrix/helmcategory"
	"imaginekube.com/imaginekube/pkg/controller/openpitrix/helmrelease"
	"imaginekube.com/imaginekube/pkg/controller/openpitrix/helmrepo"
	"imaginekube.com/imaginekube/pkg/controller/quota"
	"imaginekube.com/imaginekube/pkg/controller/serviceaccount"
	"imaginekube.com/imaginekube/pkg/controller/storage/capability"
	"imaginekube.com/imaginekube/pkg/controller/user"
	"imaginekube.com/imaginekube/pkg/controller/virtualservice"
	"imaginekube.com/imaginekube/pkg/controller/workspace"
	"imaginekube.com/imaginekube/pkg/controller/workspacerole"
	"imaginekube.com/imaginekube/pkg/controller/workspacerolebinding"
	"imaginekube.com/imaginekube/pkg/controller/workspacetemplate"
	"imaginekube.com/imaginekube/pkg/informers"
	"imaginekube.com/imaginekube/pkg/models/kubeconfig"
	...
	ldapclient "imaginekube.com/imaginekube/pkg/simple/client/ldap"
	ippoolclient "imaginekube.com/imaginekube/pkg/simple/client/network/ippool"
...
func addAllControllers(mgr manager.Manager, client k8s.Client, informerFactory informers.InformerFactory,
	cmOptions *options.ImagineKubeControllerManagerOptions,
	...
	imaginekubeInformer := informerFactory.ImagineKubeSharedInformerFactory()
	////////////////////////////////////
	// end informers
	////////////////////////////////////
	...
	if cmOptions.IsControllerEnabled("helmrelease") {
		reconcileHelmRelease := &helmrelease.ReconcileHelmRelease{
			...
			KsFactory:          informerFactory.ImagineKubeSharedInformerFactory(),
	...
	if serviceMeshEnabled {
		...
		if cmOptions.IsControllerEnabled("virtualservice") {
			vsController := virtualservice.NewVirtualServiceController(kubernetesInformer.Core().V1().Services(),
				...
				imaginekubeInformer.Servicemesh().V1alpha2().Strategies(),
				client.Kubernetes(),
				client.Istio(),
				client.ImagineKube())
		...
		if cmOptions.IsControllerEnabled("destinationrule") {
			drController := destinationrule.NewDestinationRuleController(kubernetesInformer.Apps().V1().Deployments(),
				...
				imaginekubeInformer.Servicemesh().V1alpha2().ServicePolicies(),
				client.Kubernetes(),
				client.Istio(),
				client.ImagineKube())
	...
	if cmOptions.IsControllerEnabled("loginrecord") {
		loginRecordController := loginrecord.NewLoginRecordController(
			client.Kubernetes(),
			client.ImagineKube(),
			imaginekubeInformer.Iam().V1alpha2().LoginRecords(),
			imaginekubeInformer.Iam().V1alpha2().Users(),
	...
	if cmOptions.IsControllerEnabled("globalrole") {
		if cmOptions.MultiClusterOptions.Enable {
			globalRoleController := globalrole.NewController(client.Kubernetes(), client.ImagineKube(),
				imaginekubeInformer.Iam().V1alpha2().GlobalRoles(), fedGlobalRoleCache, fedGlobalRoleCacheController)
	...
	if cmOptions.IsControllerEnabled("globalrolebinding") {
		globalRoleBindingController := globalrolebinding.NewController(client.Kubernetes(), client.ImagineKube(),
			imaginekubeInformer.Iam().V1alpha2().GlobalRoleBindings(),
	...
	if cmOptions.IsControllerEnabled("groupbinding") {
		groupBindingController := groupbinding.NewController(client.Kubernetes(), client.ImagineKube(),
			imaginekubeInformer.Iam().V1alpha2().GroupBindings(),
			imaginekubeInformer.Types().V1beta1().FederatedGroupBindings(),
			cmOptions.MultiClusterOptions.Enable)
	...
	if cmOptions.IsControllerEnabled("group") {
		groupController := group.NewController(client.Kubernetes(), client.ImagineKube(),
			imaginekubeInformer.Iam().V1alpha2().Groups(),
			imaginekubeInformer.Types().V1beta1().FederatedGroups(),
			cmOptions.MultiClusterOptions.Enable)
	...
	if cmOptions.IsControllerEnabled("cluster") {
		if cmOptions.MultiClusterOptions.Enable {
			clusterController := cluster.NewClusterController(
				...
				client.ImagineKube(),
				client.Config(),
				imaginekubeInformer.Cluster().V1alpha1().Clusters(),
				imaginekubeInformer.Iam().V1alpha2().Users().Lister(),
	...
	if cmOptions.IsControllerEnabled("nsnp") {
		if cmOptions.NetworkOptions.EnableNetworkPolicy {
			...
			nsnpController := nsnetworkpolicy.NewNSNetworkPolicyController(client.Kubernetes(),
				client.ImagineKube().NetworkV1alpha1(),
				imaginekubeInformer.Network().V1alpha1().NamespaceNetworkPolicies(),
				kubernetesInformer.Core().V1().Services(),
				kubernetesInformer.Core().V1().Nodes(),
				imaginekubeInformer.Tenant().V1alpha1().Workspaces(),
	...
	if cmOptions.IsControllerEnabled("ippool") {
		ippoolProvider := ippoolclient.NewProvider(kubernetesInformer, client.ImagineKube(), client.Kubernetes(),
			cmOptions.NetworkOptions.IPPoolType, cmOptions.KubernetesOptions)
		if ippoolProvider != nil {
			ippoolController := ippool.NewIPPoolController(imaginekubeInformer, kubernetesInformer, client.Kubernetes(),
				client.ImagineKube(), ippoolProvider)
