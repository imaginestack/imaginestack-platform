...
const (
	...
	agentImage   = "imaginekube/tower:v1.0"
	proxyService = "tower.imaginekube-system.svc"
)
...
var hostMap = map[string]string{
	"imaginekube.yaml": `
monitoring:
  endpoint: http://prometheus-operated.imaginekube-monitoring-system.svc:9090
authentication:
...
var hostCm = &corev1.ConfigMap{
	ObjectMeta: metav1.ObjectMeta{
		Namespace: constants.ImagineKubeNamespace,
		Name:      constants.ImagineKubeConfigName,
	},
	Data: hostMap,
}
                ...
                "containers": [
                    {
                        ...
                        "image": "imaginekube/ks-apiserver:v3.0.0",
                        "name": "ks-apiserver"
                    }
                ]
            }
...
func TestGeranteAgentDeployment(t *testing.T) {
	...
	informersFactory.ImagineKubeSharedInformerFactory().Cluster().V1alpha1().Clusters().Informer().GetIndexer().Add(cluster)
	...
	for _, testCase := range testCases {
		...
		t.Run(testCase.description, func(t *testing.T) {
			h := newHandler(ksclient, informersFactory.KubernetesSharedInformerFactory(),
				informersFactory.ImagineKubeSharedInformerFactory(),
				proxyService,
				"",
				agentImage)
...
func addMemberClusterResource(targetCm *corev1.ConfigMap, t *testing.T) {
	...
	_, err = c.CoreV1().Namespaces().Create(context.Background(), &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: constants.ImagineKubeNamespace}}, metav1.CreateOptions{})
	if err != nil && !errors.IsAlreadyExists(err) {
		t.Fatal(err)
	}

	_, err = c.CoreV1().ConfigMaps(constants.ImagineKubeNamespace).Create(context.Background(), targetCm, metav1.CreateOptions{})
	if err != nil && errors.IsAlreadyExists(err) {
		_, err = c.CoreV1().ConfigMaps(constants.ImagineKubeNamespace).Update(context.Background(), targetCm, metav1.UpdateOptions{})
		if err != nil {
			t.Fatal(err)
		}
	} else if err != nil {
	...
	_, err = c.AppsV1().Deployments(constants.ImagineKubeNamespace).Create(context.Background(), &deploy, metav1.CreateOptions{})
	if err != nil && !errors.IsAlreadyExists(err) {
		t.Fatal(err)
	}
}
