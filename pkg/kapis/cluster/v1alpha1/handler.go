...
import (
	...
	"imaginekube.com/api/cluster/v1alpha1"

	"imaginekube.com/imaginekube/pkg/api"
	clusterv1alpha1 "imaginekube.com/imaginekube/pkg/api/cluster/v1alpha1"
...
const (
	defaultAgentImage   = "imaginekube/tower:v1.0"
	defaultTimeout      = 10 * time.Second
	ImagineKubeApiServer = "ks-apiserver"
)
...
func newHandler(ksclient imaginekube.Interface, k8sInformers k8sinformers.SharedInformerFactory, ksInformers externalversions.SharedInformerFactory, proxyService, proxyAddress, agentImage string) *handler {
...
// created by imaginekube, we don't need to create them again. And it's a little bit inconvenient
		...
		Spec: appsv1.DeploymentSpec{
			...
			Template: corev1.PodTemplateSpec{
				...
				Spec: corev1.PodSpec{
					Containers: []corev1.Container{
						{
							...
							Command: []string{
								...
								"--keepalive=10s",
								"--imaginekube-service=ks-apiserver.imaginekube-system.svc:80",
					...
					ServiceAccountName: "imaginekube",
				},
			},
		},
	}
...
func (h *handler) updateKubeConfig(request *restful.Request, response *restful.Response) {
	...
	_, err = validateImagineKubeAPIServer(config)
	if err != nil {
		api.HandleBadRequest(response, request, fmt.Errorf("unable validate imaginekube endpoint, %v", err))
		return
	}
...
// ValidateCluster validate cluster kubeconfig and imaginekube apiserver address, check their accessibility
func (h *handler) validateCluster(request *restful.Request, response *restful.Response) {
	...
	if _, err = validateImagineKubeAPIServer(config); err != nil {
		api.HandleBadRequest(response, request, fmt.Errorf("unable validate imaginekube endpoint, %v", err))
		return
	}
...
// validateImagineKubeAPIServer uses version api to check the accessibility
func validateImagineKubeAPIServer(config *rest.Config) (*version.Info, error) {
	...
	response, err := client.Get(fmt.Sprintf("%s/api/v1/namespaces/%s/services/:%s:/proxy/kapis/version", config.Host, constants.ImagineKubeNamespace, ImagineKubeApiServer))
	if err != nil {
		return nil, err
	}
	...
	if response.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("invalid response: %s , please make sure %s.%s.svc of member cluster is up and running", ImagineKubeApiServer, constants.ImagineKubeNamespace, responseBody)
	}
...
// getMemberClusterConfig returns ImagineKube running config by the given member cluster kubeconfig
func (h *handler) getMemberClusterConfig(clientSet kubernetes.Interface) (*config.Config, error) {
	memberCm, err := clientSet.CoreV1().ConfigMaps(constants.ImagineKubeNamespace).Get(context.Background(), constants.ImagineKubeConfigName, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
...
// getHostClusterConfig returns ImagineKube running config from host cluster ConfigMap
func (h *handler) getHostClusterConfig() (*config.Config, error) {
	hostCm, err := h.configMapLister.ConfigMaps(constants.ImagineKubeNamespace).Get(constants.ImagineKubeConfigName)
	if err != nil {
		return nil, fmt.Errorf("failed to get host cluster %s/configmap/%s, err: %s", constants.ImagineKubeNamespace, constants.ImagineKubeConfigName, err)
	}
