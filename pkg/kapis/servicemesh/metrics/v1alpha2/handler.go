...
import (
	...
	"imaginekube.com/imaginekube/pkg/simple/client/kiali"
	"imaginekube.com/imaginekube/pkg/simple/client/servicemesh"
)

const (
	KubesphereNamespace      = "imaginekube-system"
	ImagineKubeServiceAccount = "imaginekube"
)
...
func NewHandler(o *servicemesh.Options, client kubernetes.Interface, cache cache.Interface) *Handler {
	if o != nil && o.KialiQueryHost != "" {
		sa, err := client.CoreV1().ServiceAccounts(KubesphereNamespace).Get(context.TODO(), ImagineKubeServiceAccount, metav1.GetOptions{})
		if err == nil {
			if len(sa.Secrets) > 0 {
