...
import (
	...
	"imaginekube.com/api/network/calicov3"
	"imaginekube.com/api/network/v1alpha1"
	...
	calicoset "imaginekube.com/imaginekube/pkg/simple/client/network/ippool/calico/client/clientset/versioned"
	calicoInformer "imaginekube.com/imaginekube/pkg/simple/client/network/ippool/calico/client/informers/externalversions"
	blockInformer "imaginekube.com/imaginekube/pkg/simple/client/network/ippool/calico/client/informers/externalversions/network/calicov3"
)
...
type provider struct {
	...
	ksclient  imaginekubeclient.Interface
...
func (p *provider) syncIPPools() error {
	...
	if err != nil {
		klog.V(4).Infof("syncIPPools: cannot list imaginekube ippools, err=%v", err)
		return err
	}
	...
	for _, calicoPool := range calicoPools.Items {
		if _, ok := existPools[calicoPool.Name]; !ok {
			...
			if err != nil {
				klog.V(4).Infof("syncIPPools: cannot create imaginekube ippools, err=%v", err)
				return err
			}
		}
	}
...
func NewProvider(k8sInformer k8sinformers.SharedInformerFactory, ksclient imaginekubeclient.Interface, k8sClient clientset.Interface, k8sOptions *k8s.KubernetesOptions) *provider {
	...
	go func() {
		for {
			if err := p.syncIPPools(); err != nil {
				klog.Infof("failed to sync calico ippool to imaginekube ippool, err=%v", err)
