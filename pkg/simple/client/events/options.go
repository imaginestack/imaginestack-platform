...
func (s *Options) AddFlags(fs *pflag.FlagSet, c *Options) {
	fs.StringVar(&s.Host, "events-elasticsearch-host", c.Host, ""+
		"Elasticsearch service host. ImagineKube is using elastic as event store, "+
	...
	fs.BoolVar(&s.BasicAuth, "events-elasticsearch-basicAuth", c.BasicAuth, ""+
		"Elasticsearch events service basic auth enabled. ImagineKube is using elastic as events store, "+
		"if it is set to true, ImagineKube will connect to ElasticSearch using provided username and password by "+
		"events-elasticsearch-username and events-elasticsearch-username. Otherwise, ImagineKube will "+
	...
	fs.StringVar(&s.IndexPrefix, "events-index-prefix", c.IndexPrefix, ""+
		"Index name prefix. ImagineKube will retrieve events against indices matching the prefix.")
