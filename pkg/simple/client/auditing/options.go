...
func (s *Options) AddFlags(fs *pflag.FlagSet, c *Options) {
	...
	fs.BoolVar(&s.BasicAuth, "auditing-elasticsearch-basicAuth", c.BasicAuth, ""+
		"Elasticsearch auditing service basic auth enabled. ImagineKube is using elastic as auditing store, "+
		"if it is set to true, ImagineKube will connect to ElasticSearch using provided username and password by "+
		"auditing-elasticsearch-username and auditing-elasticsearch-username. Otherwise, ImagineKube will "+
	...
	fs.StringVar(&s.Host, "auditing-elasticsearch-host", c.Host, ""+
		"Elasticsearch service host. ImagineKube is using elastic as auditing store, "+
		"if this filed left blank, ImagineKube will use kubernetes builtin event API instead, and"+
		" the following elastic search options will be ignored.")

	fs.StringVar(&s.IndexPrefix, "auditing-index-prefix", c.IndexPrefix, ""+
		"Index name prefix. ImagineKube will retrieve auditing against indices matching the prefix.")
