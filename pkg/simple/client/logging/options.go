...
func (s *Options) AddFlags(fs *pflag.FlagSet, c *Options) {
	fs.StringVar(&s.Host, "logging-elasticsearch-host", c.Host, ""+
		"Elasticsearch logging service host. ImagineKube is using elastic as log store, "+
		"if this filed left blank, ImagineKube will use kubernetes builtin log API instead, and"+
		" the following elastic search options will be ignored.")

	fs.BoolVar(&s.BasicAuth, "logging-elasticsearch-basicAuth", c.BasicAuth, ""+
		"Elasticsearch logging service basic auth enabled. ImagineKube is using elastic as logging store, "+
		"if it is set to true, ImagineKube will connect to ElasticSearch using provided username and password by "+
		"logging-elasticsearch-username and logging-elasticsearch-username. Otherwise, ImagineKube will "+
	...
	fs.StringVar(&s.IndexPrefix, "logging-index-prefix", c.IndexPrefix, ""+
		"Index name prefix. ImagineKube will retrieve logs against indices matching the prefix.")
