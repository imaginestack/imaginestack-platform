...
func TestParseAlertingRuleQueryParams(t *testing.T) {
	...
	req, err := http.NewRequest("GET", fmt.Sprintf("http://localhost/alerting.imaginekube.com/v2alpha1/rules?%s", queryParam), nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseAlertQueryParams(t *testing.T) {
	...
	req, err := http.NewRequest("GET", fmt.Sprintf("http://localhost/alerting.imaginekube.com/v2alpha1/alerts?%s", queryParam), nil)
	if err != nil {
		t.Fatal(err)
	}
