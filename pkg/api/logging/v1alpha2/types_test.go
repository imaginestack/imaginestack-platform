...
func TestParseQueryParameter(t *testing.T) {
	...
	req, err := http.NewRequest("GET", fmt.Sprintf("http://localhost/tenant.imaginekube.com/v2alpha1/logs?%s", defaultParam), nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	reqInterval, err := http.NewRequest("GET", fmt.Sprintf("http://localhost/tenant.imaginekube.com/v2alpha1/logs?%s", queryParamInterval), nil)
	if err != nil {
		t.Fatal(err)
	}
