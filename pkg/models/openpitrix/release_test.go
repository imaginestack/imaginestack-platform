...
func TestOpenPitrixRelease(t *testing.T) {
	...
	for _, app := range apps.Items {
		err := fakeInformerFactory.ImagineKubeSharedInformerFactory().Application().V1alpha1().HelmApplications().
	...
	for _, ver := range appvers.Items {
		err := fakeInformerFactory.ImagineKubeSharedInformerFactory().Application().V1alpha1().HelmApplicationVersions().
	...
	rlsOperator := newReleaseOperator(reposcache.NewReposCache(), fakeInformerFactory.KubernetesSharedInformerFactory(), fakeInformerFactory.ImagineKubeSharedInformerFactory(), ksClient, nil)
	...
	for _, item := range rls.Items {
		err := fakeInformerFactory.ImagineKubeSharedInformerFactory().Application().V1alpha1().HelmReleases().
