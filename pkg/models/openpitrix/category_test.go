...
func TestOpenPitrixCategory(t *testing.T) {
	...
	for _, ctg := range ctgs.Items {
		err := fakeInformerFactory.ImagineKubeSharedInformerFactory().Application().V1alpha1().HelmCategories().
...
func prepareCategoryOperator() CategoryInterface {
	...
	return newCategoryOperator(reposcache.NewReposCache(), fakeInformerFactory.ImagineKubeSharedInformerFactory(), ksClient)
}
