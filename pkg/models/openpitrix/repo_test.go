...
func TestOpenPitrixRepo(t *testing.T) {
	...
	repo := v1alpha1.HelmRepo{
		...
		Spec: v1alpha1.HelmRepoSpec{
			Name:       "test-repo",
			Url:        "https://charts.imaginekube.com/main",
			SyncPeriod: 0,
		},
	}
	...
	for _, repo := range repos.Items {
		err := fakeInformerFactory.ImagineKubeSharedInformerFactory().Application().V1alpha1().HelmRepos().
...
func prepareRepoOperator() RepoInterface {
	...
	return newRepoOperator(reposcache.NewReposCache(), fakeInformerFactory.ImagineKubeSharedInformerFactory(), ksClient)
}
