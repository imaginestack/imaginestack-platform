...
func TestTenantOperator_ListNamespaces(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			username:  admin.Name,
			result: &api.ListResult{
				Items:      []interface{}{imaginekubeSystem, defaultNamespace},
				TotalItems: 2,
			},
		},
		{
...
func TestTenantOperator_DeleteNamespace(t *testing.T) {
	...
	tests := []struct {
		...
		{
			workspace:   testWorkspace.Name,
			namespace:   imaginekubeSystem.Name,
			expectError: errors.NewNotFound(corev1.Resource("namespace"), imaginekubeSystem.Name),
		},
		{
			workspace:   testWorkspace.Name,
			namespace:   testNamespace.Name,
...
var (
	...
	imaginekubeSystem = &corev1.Namespace{
		ObjectMeta: metav1.ObjectMeta{
	...
	namespaces            = []interface{}{imaginekubeSystem, defaultNamespace, testNamespace}
...
func prepare() Interface {
	...
	k8sClient := fakek8s.NewSimpleClientset([]runtime.Object{testNamespace, imaginekubeSystem}...)
	istioClient := fakeistio.NewSimpleClientset()
	fakeInformerFactory := informers.NewInformerFactories(k8sClient, ksClient, istioClient, nil, nil, nil)

	for _, workspace := range workspaces {
		fakeInformerFactory.ImagineKubeSharedInformerFactory().Tenant().V1alpha1().
			Workspaces().Informer().GetIndexer().Add(workspace)
	}

	for _, workspaceTmpl := range workspaceTemplates {
		fakeInformerFactory.ImagineKubeSharedInformerFactory().Tenant().V1alpha2().
	...
	for _, globalRole := range globalRoles {
		fakeInformerFactory.ImagineKubeSharedInformerFactory().Iam().V1alpha2().
