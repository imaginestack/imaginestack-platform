...
import (
	...
	"imaginekube.com/imaginekube/pkg/models/monitoring/expressions"
...
type MonitoringOperator interface {
	...
	// TODO: expose ImagineKube self metrics in Prometheus format
	GetImagineKubeStats() Metrics
...
func NewMonitoringOperator(monitoringClient monitoring.Interface, metricsClient monitoring.Interface, k8s kubernetes.Interface, factory informers.InformerFactory, resourceGetter *resourcev1alpha3.ResourceGetter, op openpitrix.Interface) MonitoringOperator {
	return &monitoringOperator{
		...
		k8s:            k8s,
		ks:             factory.ImagineKubeSharedInformerFactory(),
...
func (mo monitoringOperator) GetImagineKubeStats() Metrics {
	var res Metrics
	...
	if err != nil {
		res.Results = append(res.Results, monitoring.Metric{
			MetricName: ImagineKubeClusterCount,
			Error:      err.Error(),
		})
	} else {
		res.Results = append(res.Results, monitoring.Metric{
	...
	if err != nil {
		res.Results = append(res.Results, monitoring.Metric{
			MetricName: ImagineKubeWorkspaceCount,
			Error:      err.Error(),
		})
	} else {
		res.Results = append(res.Results, monitoring.Metric{
	...
	if err != nil {
		res.Results = append(res.Results, monitoring.Metric{
			MetricName: ImagineKubeUserCount,
			Error:      err.Error(),
		})
	} else {
		res.Results = append(res.Results, monitoring.Metric{
	...
	if mo.op != nil {
		...
		if err != nil {
			res.Results = append(res.Results, monitoring.Metric{
				MetricName: ImagineKubeAppTmplCount,
				Error:      err.Error(),
			})
		} else {
			res.Results = append(res.Results, monitoring.Metric{
