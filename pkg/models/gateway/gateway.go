...
const (
	...
	gatewayPrefix           = "imaginekube-router-"
	workingNamespace        = "imaginekube-controls-system"
	globalGatewayNameSuffix = "imaginekube-system"
...
func (c *gatewayOperator) GetPods(namespace string, query *query.Query) (*api.ListResult, error) {
	...
	selector, err := labels.Parse(fmt.Sprintf("app.kubernetes.io/name=ingress-nginx,app.kubernetes.io/instance=imaginekube-router-%s-ingress", namespace))
	if err != nil {
