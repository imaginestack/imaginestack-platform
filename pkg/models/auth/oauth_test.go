	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				...
				provider: "fake",
				req:      must(http.NewRequest(http.MethodGet, "https://ks-console.imaginekube.com/oauth/callback/test?code=code1&state=100001", nil)),
			},
		...
		},
		{
			...
			args: args{
				ctx:      context.Background(),
				provider: "fake",
				req:      must(http.NewRequest(http.MethodGet, "https://ks-console.imaginekube.com/oauth/callback/test?code=code2&state=100002", nil)),
			},
			userInfo: nil,
			provider: "",
			wantErr:  true,
