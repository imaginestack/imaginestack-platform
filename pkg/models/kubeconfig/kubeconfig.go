...
import (
	...
	"imaginekube.com/imaginekube/pkg/utils/pkiutil"
)
...
const (
	...
	privateKeyAnnotation = "imaginekube.com/private-key"
...
// CreateKubeConfig Create kubeconfig configmap in ImagineKubeControlNamespace for the specified user
func (o *operator) CreateKubeConfig(user *iamv1alpha2.User) error {
	configName := fmt.Sprintf(kubeconfigNameFormat, user.Name)
	cm, err := o.configMapLister.ConfigMaps(constants.ImagineKubeControlNamespace).Get(configName)
	...
	if cm != nil {
		...
		if _, err = o.k8sClient.CoreV1().ConfigMaps(constants.ImagineKubeControlNamespace).Update(context.Background(), cm, metav1.UpdateOptions{}); err != nil {
			klog.Errorln(err)
			return err
		}
		return nil
	...
	if _, err = o.k8sClient.CoreV1().ConfigMaps(constants.ImagineKubeControlNamespace).Create(context.Background(), cm, metav1.CreateOptions{}); err != nil {
		klog.Errorln(err)
		return err
	}
...
func (o *operator) GetKubeConfig(username string) (string, error) {
	...
	configMap, err := o.configMapLister.ConfigMaps(constants.ImagineKubeControlNamespace).Get(configName)
	if err != nil {
		klog.Errorln(err)
		return "", err
	}
...
func (o *operator) UpdateKubeconfig(username string, csr *certificatesv1.CertificateSigningRequest) error {
	configName := fmt.Sprintf(kubeconfigNameFormat, username)
	configMap, err := o.k8sClient.CoreV1().ConfigMaps(constants.ImagineKubeControlNamespace).Get(context.Background(), configName, metav1.GetOptions{})
	if err != nil {
		klog.Errorln(err)
		return err
	}
	...
	_, err = o.k8sClient.CoreV1().ConfigMaps(constants.ImagineKubeControlNamespace).Update(context.Background(), configMap, metav1.UpdateOptions{})
	if err != nil {
		klog.Errorln(err)
		return err
	}
