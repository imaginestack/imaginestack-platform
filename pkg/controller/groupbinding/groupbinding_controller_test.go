...
func newUser(name string) *v1alpha2.User {
	return &v1alpha2.User{
		...
		Spec: v1alpha2.UserSpec{
			Email:       fmt.Sprintf("%s@imaginekube.com", name),
			Lang:        "zh-CN",
...
func (f *fixture) expectUpdateGroupsFinalizerAction(groupBinding *v1alpha2.GroupBinding) {
	...
	expect.Finalizers = []string{"finalizers.imaginekube.com/groupsbindings"}
	expect.Labels = map[string]string{constants.KubefedManagedLabel: "false"}
	action := core.NewUpdateAction(schema.GroupVersionResource{Group: "iam.imaginekube.com", Version: "v1alpha2", Resource: "groupbindings"}, "", expect)
...
func (f *fixture) expectPatchUserAction(user *v1alpha2.User, groups []string) {
	...
	f.ksactions = append(f.ksactions, core.NewPatchAction(schema.GroupVersionResource{Group: "iam.imaginekube.com", Resource: "users", Version: "v1alpha2"}, user.Namespace, user.Name, patch.Type(), patchData))
}
