...
import (
	...
	auditv1alpha1 "imaginekube.com/imaginekube/pkg/apiserver/auditing/v1alpha1"
	"imaginekube.com/imaginekube/pkg/apiserver/query"
	"imaginekube.com/imaginekube/pkg/apiserver/request"
	"imaginekube.com/imaginekube/pkg/client/listers/auditing/v1alpha1"
	...
	options "imaginekube.com/imaginekube/pkg/simple/client/auditing"
...
func NewAuditing(informers informers.InformerFactory, opts *options.Options, stopCh <-chan struct{}) Auditing {
	...
	a := &auditing{
		webhookLister: informers.ImagineKubeSharedInformerFactory().Auditing().V1alpha1().Webhooks().Lister(),
		devopsGetter:  devops.New(informers.ImagineKubeSharedInformerFactory()),
...
func (a *auditing) LogRequestObject(req *http.Request, info *request.RequestInfo) *auditv1alpha1.Event {
	...
	if len(e.Devops) > 0 && len(e.Workspace) == 0 {
		...
		for _, obj := range res.Items {
			...
			if d.Name == e.Devops {
				e.Workspace = d.Labels["imaginekube.com/workspace"]
