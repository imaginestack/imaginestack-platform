...
const proxyURLFormat = "/api/v1/namespaces/imaginekube-system/services/:ks-apiserver:/proxy%s"
...
func (m *multiclusterDispatcher) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	// if cluster connection is direct and imaginekube apiserver endpoint is empty
	// we use kube-apiserver proxy way
	if cluster.Spec.Connection.Type == clusterv1alpha1.ConnectionTypeDirect &&
		len(cluster.Spec.Connection.ImagineKubeAPIEndpoint) == 0 {
		...
		// We first copy req.Header['Authorization'] to req.Header['X-ImagineKube-Authorization'] before sending
		// designated cluster kube-apiserver, then copy req.Header['X-ImagineKube-Authorization'] to
		// req.Header['Authorization'] before authentication.
		req.Header.Set("X-ImagineKube-Authorization", req.Header.Get("Authorization"))

		// If cluster kubeconfig using token authentication, transport will not override authorization header,
		// this will cause requests reject by kube-apiserver since imaginekube authorization header is not
		// acceptable. Delete this header is safe since we are using X-ImagineKube-Authorization.
		...
		if httpstream.IsUpgradeRequest(req) && len(req.URL.RawQuery) != 0 {
			req.Header.Set("X-ImagineKube-Rawquery", req.URL.RawQuery)
		}
	} else {
