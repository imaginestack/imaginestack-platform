...
func WithRequestInfo(next http.Handler, resolver request.RequestInfoResolver) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		// ImagineKube supports kube-apiserver proxy requests in multicluster mode. But kube-apiserver
		...
		if len(authorization) == 0 {
			xAuthorization := req.Header.Get("X-ImagineKube-Authorization")
			if len(xAuthorization) != 0 {
				req.Header.Set("Authorization", xAuthorization)
				req.Header.Del("X-ImagineKube-Authorization")
			}
		}
		...
		if rawQuery := req.Header.Get("X-ImagineKube-Rawquery"); len(rawQuery) != 0 && len(req.URL.RawQuery) == 0 {
			req.URL.RawQuery = rawQuery
			req.Header.Del("X-ImagineKube-Rawquery")
		}
