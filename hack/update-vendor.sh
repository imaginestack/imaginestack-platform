...
function ensure_require_replace_directives_for_all_dependencies() {
  ...
  for repo in $(kube::util::list_staging_repos); do
    pushd "staging/src/imaginekube.com/${repo}" >/dev/null 2>&1
...
for repo in $(kube::util::list_staging_repos); do
  pushd "staging/src/imaginekube.com/${repo}" >/dev/null 2>&1
    if [[ ! -f go.mod ]]; then
      ...
      go mod init "imaginekube.com/${repo}"
      go mod edit -fmt
    fi
  popd >/dev/null 2>&1
done
...
if [[ ! -f go.mod ]]; then
  kube::log::status "go.mod: initialize imaginekube.com/imaginekube"
  go mod init "imaginekube.com/imaginekube"
...
kube::util::list_staging_repos \
    | while read -r X; do echo "-require imaginekube.com/${X}@v0.0.0"; done \
    | xargs -L 100 go mod edit -fmt
kube::util::list_staging_repos \
    | while read -r X; do echo "-replace imaginekube.com/${X}=./staging/src/imaginekube.com/${X}"; done \
    | xargs -L 100 go mod edit -fmt
...
for repo in $(kube::util::list_staging_repos); do
  pushd "staging/src/imaginekube.com/${repo}" >/dev/null 2>&1
    ...
    sed "s#module imaginekube.com/imaginekube#module imaginekube.com/${repo}#" \
        < "${KUBE_ROOT}/go.mod" \
        > "${KUBE_ROOT}/staging/src/imaginekube.com/${repo}/go.mod"
    # remove `require` directives for staging components (will get re-added as needed by `go list`)
    kube::util::list_staging_repos \
        | while read -r X; do echo "-droprequire imaginekube.com/${X}"; done \
        | xargs -L 100 go mod edit
    # rewrite `replace` directives for staging components to point to peer directories
    kube::util::list_staging_repos \
        | while read -r X; do echo "-replace imaginekube.com/${X}=../${X}"; done \
        | xargs -L 100 go mod edit
  popd >/dev/null 2>&1
done
...
kube::util::list_staging_repos \
    | xargs -I {} echo "imaginekube.com/{}" > "${tidy_unordered}"
...
for repo in $(tsort "${TMP_DIR}/tidy_deps.txt"); do
  pushd "${KUBE_ROOT}/staging/src/${repo}" >/dev/null 2>&1
    ...
    # disallow transitive dependencies on imaginekube.com/imaginekube
    loopback_deps=()
    kube::util::read-array loopback_deps < <(go list all 2>/dev/null | grep imaginekube.com/imaginekube/ || true)
    if [[ -n ${loopback_deps[*]:+"${loopback_deps[*]}"} ]]; then
      kube::log::error "Disallowed ${repo} -> imaginekube.com/imaginekube dependencies exist via the following imports:
...
# disallow transitive dependencies on imaginekube.com/imaginekube
loopback_deps=()
kube::util::read-array loopback_deps < <(go mod graph | grep ' imaginekube.com/imaginekube' || true)
if [[ -n ${loopback_deps[*]:+"${loopback_deps[*]}"} ]]; then
  kube::log::error "Disallowed transitive imaginekube.com/imaginekube dependencies exist via the following imports:"
...
for repo in $(kube::util::list_staging_repos); do
  rm -fr "${KUBE_ROOT}/vendor/imaginekube.com/${repo}"
  ln -s "../../staging/src/imaginekube.com/${repo}" "${KUBE_ROOT}/vendor/imaginekube.com/${repo}"
done
...
# See the OWNERS docs at https://github.com/imaginekube/imaginekube/blob/master/OWNERS
