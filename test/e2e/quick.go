...
var _ = Describe("API Test", func() {
	...
	f := framework.NewImagineKubeFramework("worksspace")
	...
	It("Should get Kubernetes objects through ks proxy", func() {
		...
		Expect(gclient.Get(context.TODO(), client.ObjectKey{Namespace: "imaginekube-system", Name: "ks-apiserver"}, deploy)).Should(Succeed())
	...
	It("Should list Kubernetes objects through ksapi by workspaces", func() {
		...
		err := gclient.List(context.TODO(), results, &opts, &client.WorkspaceOptions{Name: "imaginekube-system"})
		framework.ExpectNoError(err)
		framework.Logf("Response is %v", results)
	})
})
